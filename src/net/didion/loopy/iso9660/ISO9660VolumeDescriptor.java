...
class ISO9660VolumeDescriptor implements VolumeDescriptor, Constants {
    ...
    public boolean read(byte[] buffer) throws IOException {
        ...
        switch (type) {
            ...
            case VolumeDescriptorType.BOOTRECORD:
                Log.d("epicport-ResourceProvider", "Found boot record");
                break;
            case VolumeDescriptorType.PRIMARY_DESCRIPTOR:
            	Log.d("epicport-ResourceProvider", "Found primary descriptor");
                readPrimary(buffer);
                break;
            case VolumeDescriptorType.SUPPLEMENTARY_DESCRIPTOR:
            	Log.d("epicport-ResourceProvider", "Found supplementatory descriptor");
                readSupplementary(buffer);
                break;
            case VolumeDescriptorType.PARTITION_DESCRIPTOR:
            	Log.d("epicport-ResourceProvider", "Found partition descriptor");
                break;
            default:
            	Log.d("epicport-ResourceProvider", "Found unknown descriptor with type " + type);
        }
    ...
    void readSupplementary(byte[] buffer) throws IOException {
        ...
        if (null != enc) {
        	...
        	Log.w("epicport-ResourceProvider", "Unsupported encoding, escapeSequences: '" + this.escapeSequences + "'");
        }
    }
